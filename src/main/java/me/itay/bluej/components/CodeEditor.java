package me.itay.bluej.components;
...
public class CodeEditor extends TextArea {
	 ...
	 */
	public CodeEditor(int left, int top, int width, int height) {
			...
			if (scrollBarVisible) {
				if (lines.size() > visibleLines) {
					...
					int scrollBarHeight = Math.max(20,
							(int) ((float) visibleLines / (float) lines.size() * (float) visibleScrollBarHeight));
				...
				if (!wrapText && maxLineWidth >= width - padding * 2) {
					...
					int scrollBarWidth = Math.max(20,
							(int) ((float) visibleWidth / (float) maxLineWidth * (float) visibleScrollBarWidth));
	...
	public void handleMouseClick(int mouseX, int mouseY, int mouseButton) {
		...
		this.isFocused = GuiHelper.isMouseInside(mouseX, mouseY, xPosition, yPosition, xPosition + width,
				yPosition + height);
	...
	/**
	 * Sets the placeholder for the text area. This is the text that is shown if
	 * no text is present and gives a hint to the user what this text area is
	 * for.
	 *
	 * @param placeholder
	...
	/**
	 * Sets the highlighting for the text area. This is used, for instance,
	 * where you want particular keywords to be a different colour from the
	 * rest.
	 *
	 * @param highlight
	...
	/**
	 * Sets the maximum amount of lines that the text area can have. If the
	 * maximum lines is set to zero or below, the text area will ignore the max
	 * line property. It's suggested that this method should not be called in
	 * any other place besides the initialization of the component.
	 *
	 * @param maxLines
